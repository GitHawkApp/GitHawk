...
typedef NS_ENUM(NSUInteger, TheodoliteAsyncTransactionContainerState) {
  /**
   The async container has no outstanding transactions.
   Whatever it is displaying is up-to-date.
   */
  TheodoliteAsyncTransactionContainerStateNoTransactions = 0,
  /**
   The async container has one or more outstanding async transactions.
   Its contents may be out of date or showing a placeholder, depending on the configuration of the contained TheodoliteAsyncLayers.
   */
  TheodoliteAsyncTransactionContainerStatePendingTransactions,
};

@protocol TheodoliteAsyncTransactionContainer

/**
 @summary If YES, the receiver is marked as a container for async display, grouping all of the async display calls
 in the layer hierarchy below the receiver together in a single TheodoliteAsyncTransaction.
 ...
 */
@property (nonatomic, assign, readonly) TheodoliteAsyncTransactionContainerState ck_asyncTransactionContainerState;
...
@interface CALayer (TheodoliteAsyncTransactionContainer) <TheodoliteAsyncTransactionContainer>
/**
 ...
 */
@property (nonatomic, retain, readonly) TheodoliteAsyncTransaction *ck_asyncTransaction;
...
@interface UIView (TheodoliteAsyncTransactionContainer) <TheodoliteAsyncTransactionContainer>
@end
