...
IssueCommentSectionControllerDelegate {
    ...
    func closeAction() -> UIAlertAction? {
        ...
        return AlertAction.toggleIssue(status, issue: current?.status.pullRequest != true) { [weak self] _ in
    ...
    func lockAction() -> UIAlertAction? {
        ...
        return AlertAction.toggleLocked(locked, issue: current?.status.pullRequest != true) { [weak self] _ in
