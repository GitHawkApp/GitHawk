...
final class IssueStatusEventCell: UICollectionViewCell {
    ...
    func configure(_ model: IssueStatusEventModel) {
        ...
        let title: String
        switch model.status {
        case .reopened: title = Constants.reopened // open event only happens when RE-opening
        case .closed: title = Constants.closed
        case .locked: title = Constants.locked
        case .unlocked: title = NSLocalizedString("Unlocked", comment: "")
        case .merged: title = Constants.merged
        }
